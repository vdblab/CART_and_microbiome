...
df <- joined %>% 
  filter(genus == 'Bacteroides' & grp == 'pull_cr_d100')

    test =wilcox.test(df %>% filter(cr_d100 == 'yes') %>% pull(Relab), df %>% filter(cr_d100 == 'no') %>% pull(Relab))
    test$p.value
...
cr_res <- joined %>% 
  ...
  imap_dfr(function(.x, .y){
    test =wilcox.test(.x %>% filter(cr_d100 == 'yes') %>% pull(Relab), .x %>% filter(cr_d100 == 'no') %>% pull(Relab), exact = F)
    return(test$p.value)
  }) %>% 
  gather() %>% 
  rename(pval = value) %>% 
...
tox_res <- joined %>% 
  ...
  split(.$genus) %>% 
  imap_dfr(function(.x, .y){
    test =wilcox.test(.x %>% filter(toxicity == 'yes') %>% pull(Relab), .x %>% filter(toxicity == 'no') %>% pull(Relab), exact = F)
    return(test$p.value)
  }) %>% 
  gather() %>% 
  rename(pval = value) %>% 
...
all <- bind_rows(cr_res, tox_res)

all %>% 
  write_csv('data/lefse_sig_taxa_padj.csv')
```
...
prior <- extract.prior( mtox , n=4000 ) 

prior_tox <- list(m = 1, p = 2) %>% 
  purrr::map(function(center_){
...
# prior check: the differennce in probability
prior <- extract.prior( mcr , n=4000 ) 
...
prior <- extract.prior( gcr , n=4000 ) %>% as.tibble()  %>% select(-a) %>% mutate(grp = 'prior')
post <- extract.samples(gcr) %>% as.tibble()  %>% select(-a) %>% mutate(grp = 'post')
...
```{r}
send <- asv_annotation_blast_ag %>% 
  filter(asv_key %in% c('asv_36','asv_3','asv_27'))
send  %>% 
  write_csv('data/three_asv.csv')
```
